...
class Version:
    ...
    def __init__(self, app_kind: AppKind, major: int, minor: int, patch: int):
...
class StatusCode(IntEnum):
    ...
    def expected(self) -> Generator[None, None, None]:
        ...
        try:
            yield
            assert self == StatusCode.OK, f"Expect fail with {self} but succeed"
        except ExceptionRAPDU as e:
            try:
                status = f"{StatusCode(e.status)}"
            except ValueError:
                status = f"0x{e.status:x}"
            assert self == e.status, f"Expect fail with {self} but fail with {status}"
...
def async_thread(function: Callable[..., RESPONSE]):
    ...
    def wrapper(*args, **kwargs) -> Generator[SimpleNamespace, None, None]:
        ...
        try:
            thread_async_result = thread_pool.apply_async(function, args=args, kwds=kwargs)
            yield thread_result
...
class TezosBackend(BackendInterface):
    ...
    def _exchange(
        self,
        ins: Ins | int,
        index: Index | int = Index.FIRST,
        sig_type: SigType | int | None = None,
        payload: bytes = b"",
    ) -> bytes:
        ...
        rapdu: RAPDU = self.exchange(Cla.DEFAULT, ins, p1=index, p2=p2, data=payload)
    ...
    def _provide_public_key(self, account: Account, with_prompt: bool = False) -> bytes:
        ...
        return self._exchange(ins, sig_type=account.sig_type, payload=account.path)
    ...
    def sign(
        ...
        raise AssertionError("We should have already returned")
